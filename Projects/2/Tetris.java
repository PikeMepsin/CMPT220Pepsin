// http://www.java2s.com/Code/Java/Game/TetrisGame.htm open source game
/*Mike Pepsin
  CMPT220, Thursday, Spring 2017
  Dr. Pablo Rivas
  Project 2
  ...
  */
  ...
  public static void main(String[] args) {
    ...
    game.addPropertyChangeListener(new PropertyChangeListener() {
      ...
      game.addPropertyChangeListener(new PropertyChangeListener) ({
        public void propertyChange(PropertyChangeEvent evt) {
          if (evt.getPropertyName().equals("state")) {
            ...
            //txt.setEnabled(false);  Modifications to lines 142, 143, and 148 cause the game to start on its own without need for a username input
            //txt.requestFocus();
      ...
      btnStart.setFocusable(true); //no longer requires clicking START to start
      btnStart.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {//something in here is preventing the game from starting without input WORK TESTING NEEDED
          game.start();
        }
      });
  ...
  class Game extends Object 
 {
     ...
     **/
     ...
     private void handleGameOver() {
		 ...
		 handleStart(); //added handleStart to try to start the game as soon as it ends
     }
      ...
      */
     private void handlePause() { //this method and the next method are irrelevent because I've disabled the pause function
         thread.setPaused(true);
         state = STATE_PAUSED;
     ...
     //method that randomly generates the computer's move
     //determines what the computer does with each block
     //blocks can't go to the left or right more than 4 consecutive times in a row
     void compRun() {
       int stacked = 0; //prevents more than 3 block from being placed on the left or right
       
       long rRotate = Math.round(Math.random() * 4);
       //boolean loop = true;
       //while (loop == true) {
        for (int q = 0; q < rRotate; q++) {
          figure.rotateClockwise();
        }
        long RorL = 0;//Math.round(Math.random());
        
        long shifts = Math.round(Math.random() * 4);
        for (int test=0; test<10; test++) {
        if (RorL == 0) { //moves figure shifts to the left
          for (int l = 0; l <= shifts; l++) {
            figure.moveLeft();
          }
          stacked++;
          if (stacked == 2) {
        ...
        if (RorL == 1) { //moves figure shifts to the right
     ...
     private synchronized void handleTimer() {
         if (figure == null) {
             handleFigureStart();
             //compRun(); //randomly moves the first figure
         } else if (figure.hasLanded()) {
             handleFigureLanded();
         } else {
             //figure.moveDown();
             compRun(); //moves each figure
         }
     }
   ...
   */
   class Figure extends Object {
       ...
       * done.
       */
      public void moveLeft() { //call to move left
       ...
       * done.
       */
      public void moveRight() { //call to move right
          if (isAttached() && canMoveTo(xPos + 1, yPos, orientation)) {
              paint(null);
              xPos++;
              paint(color);
